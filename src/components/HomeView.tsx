...
/** UI copy that differs by market. Prompt terminology lives in lib/locale.ts. */
const COPY = {
  us: {
    pasteTarget: "Into Casting Networks or anywhere",
    exampleProject: "Sunshine Cola — Summer Commercial",
    exampleMeta: "Commercial · Los Angeles · Dir. Alex Park",
    exampleLead: "Male, 20s. Laid-back surfer type. Effortlessly cool, dry humor...PRINCIPAL.",
    exampleExtra: "Any gender, 18-35. Athletic builds. Beach volleyball scene...EXTRA.",
    switchHref: "/au",
    switchLabel: "Australian version",
  },
  au: {
    pasteTarget: "Into Showcast, Casting Networks or anywhere",
    exampleProject: "Sunshine Cola — Summer TVC",
    exampleMeta: "TVC · Sydney · Dir. Alex Park",
    exampleLead: "Male, 20s. Laid-back surfer type. Effortlessly cool, dry humour...PRINCIPAL.",
    exampleExtra: "Any gender, 18-35. Athletic builds. Beach volleyball scene...EXTRA.",
    switchHref: "/",
    switchLabel: "US version",
  },
} as const;

export default async function HomeView({ locale = "us" }: { locale?: Locale }) {
    ...
    <div className="min-h-screen bg-[#fafafa]">
      ...
      <nav className="bg-white border-b border-gray-200 px-6 py-3 flex items-center justify-between">
        <div className="flex items-center gap-2">
          <Link href={locale === "au" ? "/au" : "/"} className="font-bold text-lg text-gray-900">Script To Cast</Link>
          {locale === "au" && (
            <span className="text-[10px] font-semibold uppercase tracking-wide bg-gray-900 text-white px-1.5 py-0.5 rounded">AU</span>
          )}
        </div>
      ...
      <section id="tool" className="bg-white border-b border-gray-200 py-12">
        <div className="max-w-3xl mx-auto px-6">
          ...
          <SmartCreator isLoggedIn={isLoggedIn} authUnavailable={authUnavailable} locale={locale} />
        </div>
      </section>
      ...
      <section className="max-w-3xl mx-auto px-6 py-16">
        ...
        <div className="grid grid-cols-3 gap-8 text-center">
          {[
            ...
            { icon: <ClipboardCopy size={20} />, title: "Copy & paste", desc: copy.pasteTarget },
          ].map(s => (
            <div key={s.title}>
      ...
      <section className="max-w-2xl mx-auto px-6 py-16">
        ...
        <div className="bg-white border border-gray-200 rounded-xl p-5 shadow-sm space-y-4 text-sm">
          <div>
            ...
            <p className="font-semibold text-gray-900">{copy.exampleProject}</p>
            <p className="text-xs text-gray-500">{copy.exampleMeta}</p>
          </div>
            ...
            <div className="space-y-2">
              <div className="flex items-start gap-2">
                ...
                <div>
                  ...
                  <p className="text-xs text-gray-500">{copy.exampleLead}</p>
                </div>
              </div>
              <div className="flex items-start gap-2">
                ...
                <div>
                  ...
                  <p className="text-xs text-gray-500">{copy.exampleExtra}</p>
                </div>
              </div>
            </div>
          </div>
