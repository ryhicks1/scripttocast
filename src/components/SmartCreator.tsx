...
import { Upload, Copy, Check, ChevronDown, ChevronRight, FileDown, RotateCcw, Sparkles } from "lucide-react";
...
export default function SmartCreator({ isLoggedIn, initialResult }: { isLoggedIn: boolean; initialResult?: AnalysisResult }) {
      ...
      <button
        ...
        className={`inline-flex items-center gap-1 px-2 py-1 rounded text-[10px] font-semibold transition shrink-0 ${
          copied === id
            ...
            : "bg-[#1E2128] text-[#C9A84C] hover:bg-[#2A2D35]"
        }`}
      >
  ...
  if (stage === "upload") return (
    <div>
      <div
        className="border-2 border-dashed border-[#2A2D35] rounded-xl p-10 text-center cursor-pointer hover:border-[#C9A84C] transition mb-4"
        ...
        onDrop={e => { e.preventDefault(); e.currentTarget.classList.remove("border-[#C9A84C]"); addFiles(Array.from(e.dataTransfer.files)); }}
      >
      ...
      <div className="space-y-2 mb-6">
        ...
        {[
          ...
          <label key={o.key} className="flex items-center gap-2 text-sm cursor-pointer">
            <input type="checkbox" checked={opts[o.key]} onChange={() => setOpts(prev => ({ ...prev, [o.key]: !prev[o.key] }))} className="accent-[#C9A84C]" />
            {o.label}
          </label>
        ))}
      </div>
      ...
      <button
        ...
        className="w-full py-3 bg-[#C9A84C] text-[#0A0B0F] rounded-lg font-semibold text-sm disabled:opacity-40 hover:opacity-90 transition flex items-center justify-center gap-2"
      >
  ...
  if (stage === "analyzing") return (
    <div className="text-center py-20">
      <div className="w-10 h-10 border-3 border-[#1E2128] border-t-[#C9A84C] rounded-full animate-spin mx-auto mb-4" />
    ...
    <div className="space-y-4">
      ...
      <div className="bg-[#13151A] border border-[#1E2128] rounded-xl p-5">
        <div className="flex items-center justify-between mb-4">
          <h3 className="text-sm font-semibold text-[#C9A84C]">🎬 PROJECT DETAILS</h3>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          {[
            ...
            <div key={f.label} className="flex items-center justify-between bg-[#0A0B0F] rounded-lg px-3 py-2">
              <div>
        ...
        {sections.roles && (
          <div className="space-y-2">
            {result.roles.map((r, i) => (
              <div key={i} className={`bg-[#0A0B0F] border border-[#1E2128] rounded-lg p-3 transition ${doneRoles.has(i) ? "opacity-40" : ""}`}>
      ...
      {result.selfTapeInstructions?.length > 0 && (
        <div className="bg-[#13151A] border border-[#1E2128] rounded-xl p-5">
          <button onClick={() => toggleSection("instructions")} className="flex items-center justify-between w-full mb-3">
            <h3 className="text-sm font-semibold text-[#C9A84C]">🎬 SELF-TAPE INSTRUCTIONS</h3>
          ...
          {sections.instructions && (
            <div className="space-y-3">
              {result.selfTapeInstructions.map((st, i) => {
                ...
                return (
                  <div key={i} className="bg-[#0A0B0F] border border-[#1E2128] rounded-lg p-3">
      ...
      {result.formQuestions?.length > 0 && (
        <div className="bg-[#13151A] border border-[#1E2128] rounded-xl p-5">
          <button onClick={() => toggleSection("forms")} className="flex items-center justify-between w-full mb-3">
            <h3 className="text-sm font-semibold text-[#C9A84C]">📋 JOB FORM QUESTIONS</h3>
      ...
      {/* Bottom Actions */}
      <div className="sticky bottom-0 bg-[#0A0B0F]/90 backdrop-blur border-t border-[#1E2128] py-3 flex items-center gap-2 flex-wrap">
        <button onClick={copyEverything} className="flex items-center gap-1.5 px-4 py-2 bg-[#C9A84C] text-[#0A0B0F] rounded-lg text-xs font-semibold hover:opacity-90">
        ...
        {!isLoggedIn && (
          <a href="/signup" className="ml-auto flex items-center gap-1.5 px-4 py-2 border border-[#C9A84C] text-[#C9A84C] rounded-lg text-xs font-semibold hover:bg-[#C9A84C]/10">
